...
- [x] SQLite 可在服务重启后恢复历史 session 和 timeline；已通过 server-recovery 集成测试和 2026-09-06 真实同库 server 重启验收。
- [x] WebSocket 断线重连后，可用 cursor 通过 HTTP 补齐缺失事件；已有 server-recovery 集成测试，且 2026-09-06 Dashboard 刷新/实时连接人工验收通过。
...
- [x] 运行真实 manual smoke，并记录版本、命令、结果和限制；2026-09-05 最小 Claude structured smoke 已通过，2026-09-06 使用本地 gitignored 配置重复验证并确认 AgentScope 会话落库为 `completed`。
...
- [x] 只记录路径和必要 `lstat` stat（size/mtime/类型），不默认读取文件内容；删除或不可访问路径不伪造 stat。
...
- [x] 定义没有 milestone 时的隐式阶段 heuristic（按 current activity）和 confidence penalty，并输出 `implicit_phase`/`milestone_missing` reason。
