...
- A capability probe on the installed Claude Code `2.1.263` CLI (2026-09-08) confirmed that `--output-format stream-json`, `--input-format stream-json`, `--include-hook-events`, and `--include-partial-messages` are print-mode options. `--include-hook-events` is explicitly documented as working only with structured JSONL output. The normal interactive TTY therefore has no documented native event side channel that AgentScope can depend on without changing the terminal experience.
- The same probe confirmed that `--bare` disables hooks, LSP and plugin-related startup behavior. AgentScope's normal TTY path must therefore preserve the user's non-bare invocation; structured telemetry remains a separate diagnostic path rather than an implicit TTY flag.
- TTY sessions now project observer-derived activity events while running. Process, filesystem and Git signals update the current session/turn activity and ETA with their evidence source and confidence; AgentScope-started wrappers can additionally observe descendant PIDs/names without retaining command lines or environments. Delayed observer callbacks are retained as evidence but cannot overwrite a completed turn.
