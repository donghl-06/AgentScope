...
- TTY sessions now project observer-derived activity events while running. Process, filesystem and Git signals update the current session/turn activity and ETA with their evidence source and confidence; delayed observer callbacks are retained as evidence but cannot overwrite a completed turn.
